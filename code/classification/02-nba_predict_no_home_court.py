# -*- coding: utf-8 -*-
"""
Created on Tue May 03 15:03:04 2016
...
f1 = open('elo-plus-features\\05-clean-sampled-elo-plus-no-home-court.csv', 'rb')

elo_nohc = pd.read_csv(f1)
...
# drop out NAs
elo_nohc.dropna(axis=0, how='any', inplace=True)

# separate out our DV:
y = np.array(elo_nohc.win_dum)

# remove the DV, game IDs, and ref IDs from the data frame:
elo_nohc.drop(['win_dum', 'X_iscopy', 'game_id', 'date_game', 'ref_id1', 'ref_id2', 'ref_id3'], 
            axis=1, inplace=True)


# dummy out string variables:
elo_nohc = pd.get_dummies(elo_nohc)
X = elo_nohc
...
for a in alphas:
    classifier = LinearSVC(C=a, penalty='l1', dual=False)
    classifier.fit(X_norm, y)
    scores = cross_validation.cross_val_score(classifier, X_norm, y, cv=10)
    print str(a) + ':' + str(scores.mean())
    if scores.mean() > max_score:
...
# Fitting classifier with alpha=0.05
# Accuracy: 0.669 (+/- 0.06)
# Baseline accuracy (all 0's): 0.50
# Baseline accuracy (all 1's): 0.50
# Precision: 0.675 (+/- 0.07)
...
# Fitting classifier with n_estimators=5 &  learning rate=0.5
# Accuracy: 0.684 (+/- 0.08)
# Baseline accuracy (all 0's): 0.50
# Baseline accuracy (all 1's): 0.50
...
# Fitting classifier with n_estimators=1000 and a max depth of 3
# Accuracy: 0.647 (+/- 0.02)
# Baseline accuracy (all 0's): 0.50
# Baseline accuracy (all 1's): 0.50
# Precision: 0.685 (+/- 0.09)
