...
## File: "~/IST597k/NBA/code/03b-elo-rating-no-home-court-2015-16-calculation.R"

## Purpose: 
##         Fill in the new Elo ratings without home court for 
##         the 2015-16 season.
...
for(i in 1:max(uni.gms)){
  ...
  new.elo1 <- gen_new_elo_no_HC(elo_i_tm = temp$elo_i[1], 
  ...
  new.elo2 <- gen_new_elo_no_HC(elo_i_tm = temp$elo_i[2], 
...
pred <- ifelse(df$forecast >= 0.5, 1, 0)
act <- ifelse(df$game_result == "W", 1, 0)
length(which(pred==act))/nrow(df)
# 0.6691057; lower than with home court so that makes sense.


## Write out CSV:
write.csv(df, "elo-rating-cleaned-no-home-court-method-2015-16.csv", row.names = F)
